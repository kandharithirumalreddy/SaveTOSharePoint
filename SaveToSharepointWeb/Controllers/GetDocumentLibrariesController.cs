...
{
    public class GetDocumentLibrariesController : ApiController
    {
        
        [HttpPost]
        public async Task<IHttpActionResult> Post([FromBody]SiteCollectionInfo request)
        {
        ...
        private async Task<IHttpActionResult> GetSitecollectionDrives(string Id)
        {
            var bootstrapContext = ClaimsPrincipal.Current.Identities.First().BootstrapContext as BootstrapContext;
            List<DocumentLibraryInfo> drives = new List<DocumentLibraryInfo>();
            if (bootstrapContext != null)
            {
                ...
                {
                    new QueryOption("select","id,name,webUrl,driveType,description")
                };

                var sitecollectiondrives = await graphClient.Sites[Id].Drives.Request(options).GetAsync();


                foreach (var drive in sitecollectiondrives)
                {
                    drives.Add(new DocumentLibraryInfo()
                    {
                        Id=drive.Id,
                        Name=drive.Name,
                        WebUrl=drive.WebUrl,
            ...
            return ResponseMessage(new HttpResponseMessage(HttpStatusCode.OK) {Content = new StringContent(JsonConvert.SerializeObject(drives, Formatting.Indented), Encoding.UTF8, "application/json") });
        }
